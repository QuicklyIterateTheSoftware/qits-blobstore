...
public enum RepositoryType {
  ...
  /**
   * A <b>pull-through cache</b> of an upstream maven repository (default Maven Central,
   * {@code https://repo1.maven.org/maven2}), served on the same routes as {@link #MAVEN_PACKAGES}.
   *
   * <p>Separate from the hosted type rather than a flag on it, following the rule {@link #NPM_PROXY}
   * and {@link #OCI_MIRROR} already settled: cached upstream content and published content must not
   * share a namespace, and a cache must reject writes <em>by type</em> rather than by configuration.
   * So a {@code PUT} here is {@code 405} because of what this constant is, and no repository can
   * drift from one meaning to the other — {@code ArtifactRepositoryService.ensure} makes a type
   * immutable.
   *
   * <p>Cached files live in ordinary {@code maven_artifact} rows under the proxy's own repository
   * name, so the census, the explorer and the blob store need no new code — a cached jar is a
   * deployed jar to all three, told apart by its repository's type. The one thing that needs a table
   * of its own is {@code maven-metadata.xml}: it is the only maven document that mutates, so it is
   * cached with a TTL in {@code maven_proxy_metadata} rather than being an immutable path.
   *
   ...
   * /artifacts/docs/} by {@code eu.wohlben.qits.docs} and fronted for humans by qits-docs.
   *
   * <p>A protocol type on the {@link #OCI_IMAGES} pattern, and everything that javadoc says about
   * empty profiles and a zero cap holds verbatim: a bundle arrives as one {@code .tar.gz} on a
   * streaming {@code PUT} and its entries go straight to {@code BlobStore}, so there is no single
   * media type to sniff and no metadata to require. The real cap is {@code
   * qits.artifacts.docs.max-bundle-size}.
   *
   * <p><b>It names a role, not a file format</b>, the rule {@link #DAEMON_BINARIES} states. A
   * generic {@code static-files} type would collect anything merely servable and could answer none
   * of the three questions a type has to answer. This one does: <b>who publishes</b> — the release
   * pipeline of a repository that has documentation to ship, declaring {@code {type: docs}} in its
   * trigger file; <b>what GC keeps</b> — the newest versions of every site, the rest ageing out
   * unaccessed; <b>what pins an entry</b> — nothing outside this service yet, because qits-docs is
   * a stateless view that resolves {@code latest} from these rows rather than holding a pointer of
   * its own.
   *
   * <p><b>A version is the unit, never a file.</b> {@code docs_site} is the identity and the only
   * thing a GC strategy sees; {@code docs_file} rows hang off it and go with it. A bundle's files
   * are ordinary deduplicated blobs, so an asset shared with a version that survives is kept by
   * that version's rows — the sweep answers what may be unlinked, as it does for every type here.
   *
   * <p>Versions are immutable — re-publishing one is {@code 409}, the stance {@link
   * #DAEMON_BINARIES} takes and for the same reason: a second publish at one version means the
   * version was reused or the release ran twice.
   */
  DOCS(Set.of(), Set.of(), 0L);
